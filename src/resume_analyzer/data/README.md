...
|------|-------------|------|
| `manual_gold_dataset.csv` | **Primary labeled dataset** - human-reviewed labels and sub-scores | 29,838 |
...
```python
import pandas as pd
df = pd.read_csv('data/manual_gold_dataset.csv')
X = df['cv_text']      # Input text
y = df['career_label'] # Target: career field
```
...
import pandas as pd
df = pd.read_csv('data/manual_gold_dataset.csv')
X = extract_features(df['cv_text'])  # Custom features
y = df['quality_label']              # Target: 0=weak, 1=average, 2=strong
```

## Notes

- **Scoring Method**: The gold dataset includes human-reviewed quality labels and sub-scores
- **Fallback**: `labeled_resumes.csv` remains available if you need the auto-labeled training set
