        ...
        if candidate.is_dir():
            for f in candidate.iterdir():
                if f.suffix in {".pkl", ".json"}:
                    return candidate
    raise FileNotFoundError("Could not locate models/ directory for career model")

MODEL_DIR = _find_models_dir()
MODEL_PATH = str(MODEL_DIR / "career_model.pkl")
CLASSES_PATH = str(MODEL_DIR / "career_classes.pkl")
