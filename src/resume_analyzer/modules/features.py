...
import re
from modules.extractor import normalize_text
...
def extract_features(text: str, career_label: str | None = None) -> dict:
    """
    Takes clean resume text.
    career_label is accepted for training/runtime compatibility, but the
    current feature set remains field-agnostic so the saved scorer model
    keeps the same 27-feature contract.
    ...
    has_linkedin = int(bool(re.search(
        r"linkedin\.com", raw_lower
    )))
    has_github = int(bool(re.search(
        r"(github\.com|github\.io)", raw_lower
    )))
    has_portfolio = int(bool(re.search(
        r"(portfolio|behance\.net|dribbble\.com|personal site|mysite|website|portfolio\.|portfolio/|portfolio\b)", raw_lower
    )))
...
def extract_keywords(text: str, top_n: int = 10) -> list:
    ...
    words = re.findall(r"\b[a-zA-Z]{3,}\b", normalize_text(text))
