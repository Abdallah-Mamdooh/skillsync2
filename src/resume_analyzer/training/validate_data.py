...
Responsibilities:
- Validate manual_gold_dataset.csv as the primary labeled dataset
- Optionally validate labeled_resumes.csv as the auto-labeled fallback
...
def main() -> None:
    # Validate manual gold dataset first
    validate_file(GOLD_PATH, "Manual Gold Dataset", encoding="latin-1")

    # Validate auto-labeled fallback if present
    if os.path.exists(AUTO_PATH):
        validate_file(AUTO_PATH, "Labeled Resumes (auto-scored)", encoding="utf-8")

        # Cross-check: confirm both files share the same column schema
        df_auto = pd.read_csv(AUTO_PATH, nrows=1, dtype=str, encoding="utf-8")
        ...
        shared_cols  = set(df_auto.columns) & set(df_gold.columns)
        only_in_auto = set(df_auto.columns) - set(df_gold.columns)
        only_in_gold = set(df_gold.columns) - set(df_auto.columns)

        print("=== Schema Comparison ===")
        print(f"Shared columns:         {sorted(shared_cols)}")
        if only_in_auto:
            print(f"Only in auto-labeled:   {sorted(only_in_auto)}")
        if only_in_gold:
            print(f"Only in gold dataset:   {sorted(only_in_gold)}")
        print()
    else:
        print(f"INFO: Auto-labeled fallback not found at: {AUTO_PATH}")
        print("      Skipping fallback validation.\n")
